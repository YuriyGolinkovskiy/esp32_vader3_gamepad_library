...
static constexpr unsigned long STATUS_INTERVAL_MS = 2000;
static constexpr int STICK_DEADZONE = 10;
static constexpr int TRIGGER_DEADZONE = 5;
...
void loop() {
  ...
  if (!isConnected) {
    ...
    if (millis() - lastMsg > STATUS_INTERVAL_MS) {
  ...
  if (abs(lx - lastLX) > STICK_DEADZONE || abs(ly - lastLY) > STICK_DEADZONE) {
    Serial.printf("🎮 Левый стик: X=%4d Y=%4d\n", lx, ly);
    lastLX = lx; lastLY = ly;
  }

  if (abs(rx - lastRX) > STICK_DEADZONE || abs(ry - lastRY) > STICK_DEADZONE) {
  ...
  if (abs(lt - lastLT) > TRIGGER_DEADZONE) {
    Serial.printf("🎯 Левый триггер: %d\n", lt);
    lastLT = lt;
  }

  if (abs(rt - lastRT) > TRIGGER_DEADZONE) {
